...
var KTAccentWord = function (data, result) {
    ...
    this.sharesPreviousAccent = false;
    this.notes = [];
};
...
KTAccentWord.prototype.slice = function (start, end) {
    return this.result ? this.result.slice(start, end) : undefined;
};
...
KTAccentWord.prototype.addNote = function (tag, info) {
    return this.notes.push({ tag: tag, info: info });
};
...
function hasVoiclessVowel(string) {
    var has = false;
    var position = string.length - 1;
    while (!has && position > 0) {
        has = isAtVoiclessVowel(string, position);
        position--;
    }
...
var utilities = {
    ...
    hasVoiclessVowel: hasVoiclessVowel
};
...
KTAccentSession.prototype.internalParse = function() {
    ...
    if (this.textArray.length == 0) {
        var finalPronunciation = this.resultArray.map((x, i, a) => {
            var addDivider = true;
        ...
        return this.callback(finalPronunciation, this.resultArray);
    }
